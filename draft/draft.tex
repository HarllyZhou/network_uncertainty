...
\begin{align}\label{eq:credit_spread_rule}
    ...
    \psi U_t,
\end{align}
for some $\psi>0$, where $U_t$ is the aggregate uncertainty measure à la [].
\begin{align*}
    U_t = \sum_{i=1}^N \omega^U_{i,t-1}\left(\sigma^R_{i,t|t-1}\right)^2
    =
    \sum_{i=1}^N \omega^U_{i,t} \operatorname{Var}_{t-1}\left(R_{i,t}\right) = \sum_{i=1}^N \omega^U_{i,t} p_{i,t} \tilde{y}_{i,t} \sigma^A_{i,t|t-1}
\end{align*}
is the aggregate variance of firm revenue with $\omega^U_{i,t-1}$ being the weights.
