...
\begin{align}\label{eq:credit_spread_rule}
    ...
    \bar{s}
    +
    \psi \left(\sigma^R_{i,t|t-1}\right)^2,
\end{align}
for some $\psi>0$, where 
\begin{align*}
    \sigma^R_{i,t|t-1}
    =
    \sqrt{\operatorname{Var}_{t-1}\left(R_{i,t}\right)} = p_{i,t} \tilde{y}_{i,t} \sigma^A_{i,t|t-1}
\end{align*}
is the conditional standard deviation of firm revenue.



The intermediary's nominal balance-sheet constraint is
...
\begin{align*}
    \Pi^f_t = \sum_{i=1}^N \min\left\{R_{i,t}, (1+i^L_{i,t})b_{i,t}\right\} - (1+i_{t-1})D_{t-1}.
\end{align*}
...
subject to
\begin{align*}
    \mathbb{P}_{t-1} \left( R_{i,t} < (1+i^L_{i,t})b_{i,t} \right) \leq \delta,
\end{align*}
...
\begin{align*}
    \mathbb{P}_{t-1} \left( R_{i,t} < (1+i^L_{i,t})b_{i,t} \right) &= \mathbb{P}_{t-1} \left( \log A_{i,t} < \log\left((1+i^L_{i,t})b_{i,t}\right) - \log p_{i,t}\tilde{y}_{i,t} \right) \\
    &= \Phi\left(\frac{\log\left((1+i^L_{i,t})b_{i,t}\right) - \log p_{i,t}\tilde{y}_{i,t} - \bar{a}_i}{\sigma_{i,t-1}}\right)\\
    &\leq \delta.
\end{align*}
This is equivalent to
\begin{align*}
    b_{i,t} \leq \frac{\mu^R_{i,t|t-1}}{1+i^L_{i,t}}\exp\left(-\frac{1}{2}{\sigma^2_{i,t-1}}-z_{1-\delta}\sigma_{i,t-1}\right) = \frac{\exp\left(-\frac{1}{2}\sigma^2_{i,t-1}\right)}{1+i^L_{i,t}} \mu^R_{i,t|t-1} \exp(-z_{1-\delta}\sigma_{i,t-1}).
\end{align*}
...
\begin{align*}
    \mathbb{E}_{t-1}\Pi^f_t = \sum_{i=1}^N \left[p_{i,t}\mu^A_{i,t|t-1}\tilde{y}_{i,t}\left(1-\Phi(q_2)\right) + (1+i^L_{i,t})b_{i,t} \Phi(q_1)\right] - (1-i_{t-1})D_{t-1},
\end{align*}
where
\begin{align*}
    q_1 &= \frac{\bar{a}_i - \log\left((1+i^L_{i,t})b_{i,t}\right) + \log \left(p_{i,t}\tilde{y}_{i,t}\right)}{\sigma_{i,t-1}},\\
    q_2 &= \frac{\bar{a}_i + \sigma^2_{i,t-1} - \log\left((1+i^L_{i,t})b_{i,t}\right) + \log \left(p_{i,t}\tilde{y}_{i,t}\right)}{\sigma_{i,t-1}} = q_1 + \sigma_{i,t-1}.
\end{align*}
...
\begin{align*}
    (1+i^L_{i,t})b_{i,t} = p_{i,t}\mu^A_{i,t|t-1}\tilde{y}_{i,t}\exp\left(-\frac{1}{2}\sigma^2_{i,t-1} - z_{1-\delta}\sigma_{i,t-1}\right).
\end{align*}
This condition and the zero-expected profit condition for loan $i$ together imply that
\begin{align*}
    p_{i,t}\mu^A_{i,t|t-1}\tilde{y}_{i,t}\left(1-\Phi\left(z_{1-\delta}+\sigma_{i,t-1}\right)\right) + (1+i^L_{i,t})b_{i,t}(1-\delta) = (1+i_{t-1})b_{i,t}.
\end{align*}
...
\end{align*}
where $\Phi(\cdot)$ is the cumulative distribution function of the standard normal distribution with probability density function $\phi(\cdot)$ and $q_1$ and $q_2$ are defined as in the proof of Lemma \ref{lemma:conditional_mean_std_productivity_benchmark}.
Note that
\begin{align*}
    \frac{\phi(q_2)}{\phi(q_1)} &= \exp\left({-\sigma_{i,t-1}q_1 - \frac{\sigma^2_{i,t-1}}{2}}\right) = \frac{(1+i^L_{i,t})b_{i,t}}{\mu^A_{i,t|t-1}p_{i,t}\tilde{y}_{i,t}}.
\end{align*}
...
\begin{lemma}[Binding Working-Capital Constraint]
...
\begin{enumerate}
    ...
    \item The monetary authority sets $i_t$ and $X_t$ according to the benchmark policy process.
