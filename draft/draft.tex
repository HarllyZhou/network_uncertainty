...
\begin{align}\label{eq:borrowing_constraint}
    \ell_{i,t} \leq \kappa R_{i,t-1} \exp\left(-\frac{1}{2}\sigma^2_{i,t-1}-\theta \sigma_{i,t-1}\right).
\end{align}
...
\begin{lemma}[Optimal Lending Amount]\label{lemma:optimal_lending_amount}
    ...
    \begin{align*}
        ...
        \begin{array}{ll}
            ...
            \text{ and } \mathbb{P}_{t-1} \left( R_{i,t} < (1+i^L_{t})\ell_{i,t} \right) < \delta, \\[1.2em]
            \dfrac{1+i_{t-1}}{1+i^L_t},
            & \text{if } \sum_{i=1}^N \ell_{i,t}=D_{t-1}
            \text{ and } \mathbb{P}_{t-1} \left( R_{i,t} < (1+i^L_{t})\ell_{i,t} \right) < \delta, \\[1.2em]
            1-\delta,
            & \text{if } \mathbb{P}_{t-1} \left( R_{i,t} < (1+i^L_{t})\ell_{i,t} \right) = \delta.
        \end{array}
        \right.
    \end{align*}
\end{lemma}
...
\begin{align}
    [\text{ Euler }]:\, &1 = \beta \mathbb{E}_t \left[\frac{1+i^D_t}{1+\pi_{t+1}}\frac{C_t}{C_{t+1}}\right], \label{eq:hh_foc_euler} \\
    [\text{ Labour supply }]:\, &\frac{W_t}{P_t} = \chi C_t N_t^{\varphi}. \label{eq:hh_foc_labour}
\end{align}
...
\end{align*}
which is linear in $b_{i,t}$. Since the result of the maximization operator is non-negative, maximization of the expected profit requires $b_{i,t}$ to be at the maximum allowed level. Therefore, the borrowing constraint must be binding. Define this maximum level as $\bar{b}_{i,t} := \kappa R_{i,t-1} \exp \left(-\frac{1}{2}\sigma_{i,t-1}^2 - \theta \sigma_{i,t-1}\right)$. We have $b_{i,t} = \bar{b}_{i,t}$ at equilibrium.
...
\begin{lemma}[Borrowing Constraint Binding]\label{lemma:borrowing_constraint_binding}
    The borrowing constraint \eqref{eq:borrowing_constraint} is binding in equilibrium.
\end{lemma}
\begin{proof}
...
Moreover, substituting the expression for $\lambda^y_{2i,t}$ into the price-cost ratio, we have
\begin{align}\label{eq:price_cost_ratio_benchmark}
    \frac{p_{i,t}}{{mc}_{i,t}} = \frac{(1+i^L_t)}{\mu^A_{i,t|t-1}} \frac{\phi(q_{1i,t})}{\phi(q_{2i,t})} = (1+i^L_t) \exp \left(-\bar{a}_i + q_{1i,t}\sigma_{i,t-1}\right).
\end{align}
By the definition of $q_{1i,t}$, we have
\begin{align*}
    \mathbb{P}_{t-1} \left(R_{i,t} \geq (1+i^L_t) b_{i,t}\right) = \Phi(q_{1i,t}).
\end{align*}
Therefore, from a partial equilibrium perspective, the sign of the uncertainty effect depends on the firm's repayment position. When the firm repays in more than half of states, the repayment threshold lies below median revenue, and greater uncertainty raises the price-cost ratio required to maintain that repayment position. When the firm repays in less than half of states, the firm is effectively relying on upper-tail outcomes, and limited liability makes uncertainty valuable, so the required price-cost ratio falls.
