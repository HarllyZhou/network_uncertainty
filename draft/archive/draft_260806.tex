...
Firm $i$ is a monopolistically competitive producer of differentiated good $i$. It uses labour and intermediate inputs from other sectors. The quantity of good $j$ used in the production of good $i$ is $x_{ij,t}$. Firms take wages and input prices as given. At the beginning of the period, firm $i$ sets its nominal price $p_{i,t}$ before productivity $A_{i,t}$ realizes.
Final demand for good $i$ is implied by the household CES aggregator:
\begin{align}\label{eq:final_demand_benchmark}
    c_{i,t}
    =
    \rho_i
    \left(
...
\end{align*}
so that the sum of each row of $\tilde{\Omega}_t$ is 1. Denote the un-normalized production network by $\Omega_t$ where the $(i,j)$-th entry is $\omega_{ij,t}$.
...
\end{align}
Here $\bar a_i$ is the sector-specific average log productivity, $\sigma_{i,t}$ is the standard deviation scale of log productivity, and $\epsilon_{i,t}$ is the level shock. Let
\begin{align*}
...
Given information at the end of period $t-1$, productivity in period $t$ is log-normally distributed, with conditional mean and standard deviation
\begin{align*}
    \mu^A_{i,t|t-1}
    =
    \exp\left(
    \bar a_i+\frac{1}{2}\sigma_{i,t-1}^2
    \right),
    \qquad
    \sigma^A_{i,t|t-1}
    =
    \mu^A_{i,t|t-1}
    \sqrt{
    \exp(\sigma_{i,t-1}^2)-1
    }.
\end{align*}
\end{lemma}

\begin{proof}
Conditional on information at the end of period $t-1$, $\log A_{i,t}$ is normally distributed with mean $\bar a_i$ and variance $\sigma_{i,t-1}^2$. The result follows from the standard moments of a log-normal random variable.
\end{proof}
...
\begin{align}\label{eq:firm_financial_constraint}
    ...
    b_{i,t}.
\end{align}
The firm is also subject to a borrowing constraint, which will be specified in Section \ref{sec:financial_intermediary}.
...
\begin{align}\label{eq:firm_revenue}
    \mathcal R_{i,t}
    =
    p_{i,t}y_{i,t}.
\end{align}
The firm repays its debt with gross interest rate $1+i^L_{i,t}$. Under limited liability, its nominal profit is
\begin{align}\label{eq:firm_profit_limited_liability}
    \Pi^y_{i,t}
    =
    \max
    \left\{
...
There are three market in the economy: the goods market, the labour market, and the financial market. The goods market clears when condition \eqref{eq:goods_market_clear} holds:
\begin{align*}
